  ...
  const navBtn = (id: Screen | string, label: string, match?: (s: string) => boolean) => (
    <button
      ...
      className={`px-3 py-1.5 text-[12px] rounded-md transition-all duration-200 ${
        (match ? match(currentScreen) : currentScreen === id)
...
function AdminOverview({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-10">
      <div className="mb-10">
        <h1 className="text-[28px] font-semibold text-stone-800 tracking-[-0.02em] leading-tight">Admin Settings</h1>
        <p className="text-[14px] text-stone-400 mt-1.5">Configure system settings, permissions, and workflows</p>
      </div>

      <div className="space-y-8">
        {sections.map((section, idx) => (
          <div key={idx}>
            <h2 className="text-[11px] font-medium text-stone-400 uppercase tracking-[0.08em] mb-3">{section.title}</h2>
              ...
              {section.cards.map((card) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-[14px] font-medium text-stone-800 group-hover:text-stone-900 transition-colors">{card.label}</div>
                    <div className="text-[12px] text-stone-400 mt-0.5">{card.description}</div>
                  </div>
                  <span className="text-stone-300 group-hover:text-stone-500 group-hover:translate-x-0.5 transition-all text-[14px] ml-4 flex-shrink-0">→</span>
                </button>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800">Module Management</h1>
          <p className="text-sm text-stone-500">Enable or disable system modules</p>
        </div>
          ...
          {modules.map((module) => (
            <div key={module.id} className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-lg ${module.enabled ? 'bg-primary/10' : 'bg-stone-100'} flex items-center justify-center`}>
                  <Settings className={`w-5 h-5 ${module.enabled ? 'text-primary' : 'text-stone-400'}`} />
                </div>
                <div>
                  <p className="font-medium text-stone-800">{module.name}</p>
                  {module.locked && (
                    <p className="text-xs text-stone-500">Core module (always enabled)</p>
                  )}
                </div>
              </div>
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  module.enabled ? 'bg-primary' : 'bg-stone-300'
  ...
  return (
    <div className="p-8">
      <button
        ...
        className="flex items-center gap-2 text-sm text-stone-700 hover:text-stone-900 mb-6"
      >
        ← Back to Modules
      </button>

      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Sidebar Preview</h1>
        <p className="text-sm text-stone-500">How the sidebar appears with Finance disabled</p>
      </div>
...
function PermissionsMatrix({ onNavigate, showDrawer, setShowDrawer }: { onNavigate: (screen: Screen) => void; showDrawer: boolean; setShowDrawer: (show: boolean) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Roles & Permissions</h1>
        <p className="text-sm text-stone-500">Configure access control by role and module</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-stone-50 border-b border-stone-200">
              <th className="text-left px-4 py-3 text-xs font-semibold text-stone-700 sticky left-0 bg-stone-50">
                Module
              </th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-stone-700">
                Permission
              </th>
              {roles.map(role => (
                <th key={role} className="text-center px-2 py-3 text-xs font-semibold text-stone-700">
                  {role}
                </th>
              ))}
            </tr>
            ...
            {modules.flatMap(module => 
              module.permissions.map((permission, idx) => (
                <tr 
                  ...
                  className={`${idx === 0 ? 'bg-stone-50/50' : ''} hover:bg-stone-50 cursor-pointer`}
                  onClick={() => setShowDrawer(true)}
                >
                  {idx === 0 && (
                    <td
                      rowSpan={module.permissions.length}
                      className="px-4 py-3 font-medium text-stone-800 border-r border-stone-200 sticky left-0 bg-white"
                    >
                      {module.name}
                    </td>
                  )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-end">
          <div className="bg-white w-full max-w-md h-full overflow-y-auto">
            <div className="p-6 border-b border-stone-200 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-stone-800">Sales Permissions</h2>
              <button onClick={() => setShowDrawer(false)} className="p-2 hover:bg-stone-100 rounded">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-6">
              <div>
                <h3 className="font-semibold text-stone-800 mb-3">Fine-Grained Permissions</h3>
                <div className="space-y-2">
...
function CustomFieldsBuilder({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Custom Fields</h1>
        <p className="text-sm text-stone-500">Add custom fields to modules and entities</p>
      </div>

      <div className="bg-white rounded-lg border border-stone-200 p-6 mb-6">
        <label className="block text-sm font-medium text-stone-700 mb-2">Select Module</label>
        <select
      ...
      <div className="bg-white rounded-lg border border-stone-200 mb-4">
        <div className="px-6 py-4 border-b border-stone-200 flex items-center justify-between">
          <h3 className="font-semibold text-stone-800">Custom Fields for {selectedModule}</h3>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg">
            <div className="px-6 py-4 border-b border-stone-200">
              <h2 className="text-xl font-semibold text-stone-800">Add Custom Field</h2>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2">Field Name</label>
                <input type="text" className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm" placeholder="e.g., Project Budget" />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2">Field Type</label>
      ...
      <div className="bg-white rounded-lg border border-stone-200 p-6">
        <h3 className="font-semibold text-stone-800 mb-4">Field Types Available</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {['Text', 'Number', 'Date', 'Dropdown', 'Checkbox', 'Radio', 'Textarea', 'URL'].map(type => (
            <div key={type} className="p-3 border border-stone-200 rounded-lg text-center text-sm text-stone-700">
              {type}
            </div>
          ))}
        </div>
...
function PortalSettings({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Portal Settings</h1>
        <p className="text-sm text-stone-500">Configure client, employee, and freelancer portal features</p>
      </div>
...
function ClientPortalSettings({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Client Portal Settings</h1>
        <p className="text-sm text-stone-500">Configure what clients can see and do</p>
      </div>

      <div className="bg-white rounded-lg border border-stone-200 mb-6">
        <div className="px-6 py-4 border-b border-stone-200">
          <h3 className="font-semibold text-stone-800">Portal Features</h3>
        </div>
        <div className="divide-y divide-stone-200">
          {[
            ...
            <div key={key} className="p-4 flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  features[key as keyof typeof features] ? 'bg-indigo-600' : 'bg-stone-300'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
      ...
      <div className="bg-white rounded-lg border border-stone-200 p-6">
        <h3 className="font-semibold text-stone-800 mb-4">Client Roles Permissions</h3>
        <div className="space-y-3">
          <div className="p-4 bg-stone-50 rounded-lg">
            <p className="font-medium text-stone-800 mb-2">Client Admin</p>
            <p className="text-sm text-stone-600">Full access to all portal features, can manage other client users</p>
          </div>
          <div className="p-4 bg-stone-50 rounded-lg">
            <p className="font-medium text-stone-800 mb-2">Client User</p>
            <p className="text-sm text-stone-600">View-only access, can submit forms and view assigned content</p>
          </div>
        </div>
      </div>
    </div>
...
function EmployeePortalSettings({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Employee Portal Settings</h1>
        <p className="text-sm text-stone-500">Configure what employees can see and do</p>
      </div>
...
function FreelancerPortalSettings({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Freelancer Portal Settings</h1>
        <p className="text-sm text-stone-500">Configure what freelancers can see and do</p>
      </div>
...
function ApprovalsConfiguration({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Approvals Configuration</h1>
        <p className="text-sm text-stone-500">Configure approval workflows and default approvers</p>
      </div>

      <div className="bg-white rounded-lg border border-stone-200 mb-6">
        <div className="px-6 py-4 border-b border-stone-200">
          <h3 className="font-semibold text-stone-800">Approval Types</h3>
        </div>
        <div className="divide-y divide-stone-200">
          {[
            ...
            <div key={key} className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex-1">
                  <p className="font-medium text-stone-800">{label}</p>
                  <p className="text-sm text-stone-600">Default approver: {approver}</p>
                </div>
                <button
                  onClick={() => toggleApprovalType(key as keyof typeof approvalTypes)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    approvalTypes[key as keyof typeof approvalTypes] ? 'bg-primary' : 'bg-stone-300'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
              ...
              <div className="flex items-center gap-4">
                <select className="px-3 py-1 text-sm border border-stone-200 rounded-lg">
                  <option>Owner</option>
                ...
                <input
                  ...
                  className="w-32 px-3 py-1 text-sm border border-stone-200 rounded-lg"
                />
              </div>
            </div>
          ))}
      ...
      <div className="bg-white rounded-lg border border-stone-200 p-6">
        <h3 className="font-semibold text-stone-800 mb-3">Approval Rules</h3>
...
function IntegrationsSettings() {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Integrations</h1>
        <p className="text-sm text-stone-500">Connect external services and tools</p>
      </div>
                  ...
                  {integration.id === 'teams' && (
                    <>
                      <label className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm text-stone-800">Channel notifications</span>
                      </label>
                    </>
                  )}
                  <p className="text-xs text-stone-500 mt-2">Last synced: 5 minutes ago</p>
                </div>
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500">System activity and compliance tracking</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-stone-200 mb-4">
        <div className="p-4 border-b border-stone-200">
          <div className="flex items-center gap-3">
            ...
            />
            <select className="px-3 py-2 text-sm border border-stone-200 rounded-lg">
      ...
      <div className="bg-white rounded-lg border border-stone-200 p-6">
        <h3 className="font-semibold text-stone-800 mb-2">Audit Log Retention</h3>
        <p className="text-sm text-stone-600">Events are retained for 90 days. Exports available on request.</p>
      </div>
    </div>
  );
}
...
function GDPRSettings({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">GDPR & Data Privacy</h1>
        <p className="text-sm text-stone-500">Configure data retention and privacy policies</p>
      </div>

      <div className="space-y-4">
        <div className="bg-white rounded-lg border border-stone-200 p-6">
          <h3 className="font-semibold text-stone-800 mb-4">Data Retention</h3>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-3 bg-stone-50 rounded-lg">
                ...
                <input
                  ...
                  className="w-32 px-3 py-1 text-sm border border-stone-200 rounded-lg text-right"
                />
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-lg border border-stone-200 p-6">
          <h3 className="font-semibold text-stone-800 mb-4">Data Export Settings</h3>
          <p className="text-sm text-stone-600 mb-3">Users can request data exports from their portal profile</p>
          <label className="flex items-center gap-2">
            <input type="checkbox" defaultChecked className="w-4 h-4 rounded border-stone-300 text-primary" />
            <span className="text-sm text-stone-700">Allow self-serve data exports</span>
          </label>
        </div>

        <div className="bg-white rounded-lg border border-stone-200 p-6">
          <h3 className="font-semibold text-stone-800 mb-4">Deletion Policy</h3>
          <p className="text-sm text-stone-600">
...
function EntitySchemaManager({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Entity Schema Manager</h1>
        <p className="text-sm text-stone-500">Configure fields and layouts for entities</p>
      </div>
        ...
        {entities.map((entity) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold text-stone-800">{entity.name}</h3>
              <Database className="w-5 h-5 text-stone-400" />
            </div>
            <p className="text-sm text-stone-600">{entity.fields} fields configured</p>
          </button>
        ))}
      </div>
    </div>
...
function EntityFieldsList({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800">Clients - Fields</h1>
          <p className="text-sm text-stone-500">Manage fields, visibility, and layout</p>
        </div>
        <div className="flex gap-2">
        ...
        <div className="divide-y divide-stone-200">
          {fields.map((field) => (
            <div key={field.id} className="p-4 flex items-center gap-4">
              <button className="text-stone-400 hover:text-stone-600 cursor-move">
...
function FieldLayoutDesigner({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Field Layout Designer</h1>
        <p className="text-sm text-stone-500">Drag fields into sections</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {[
          ...
          <div key={idx} className="bg-white rounded-lg border border-stone-200 p-4">
            <h3 className="font-semibold text-stone-800 mb-3">{section.title}</h3>
            <div className="space-y-2">
              {section.fields.map((field, i) => (
                <div key={i} className="p-3 bg-stone-50 rounded-lg flex items-center gap-2 cursor-move">
                  <GripVertical className="w-4 h-4 text-stone-400" />
                  <span className="text-sm text-stone-800">{field}</span>
                </div>
              ))}
            </div>
          </div>
...
function PipelineManager({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Pipeline Manager</h1>
        <p className="text-sm text-stone-500">Configure pipeline stages for deals, jobs, candidates, and projects</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {pipelines.map((pipeline) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold text-stone-800">{pipeline.name}</h3>
              <GitBranch className="w-5 h-5 text-stone-400" />
            </div>
            <p className="text-sm text-stone-600">{pipeline.stages} stages configured</p>
          </button>
        ))}
      </div>
    </div>
...
function PipelineStageEditor({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800">Deals – Project Pipeline</h1>
          <p className="text-sm text-stone-500">Manage stages, colors, and rules</p>
        </div>
        <BonsaiButton size="sm" variant="primary" icon={<Plus />}>
          Add Stage
        </BonsaiButton>
      ...
      <div className="bg-white rounded-lg border border-stone-200 p-6">
        <h3 className="font-semibold text-stone-800 mb-3">Pipeline Rules</h3>
        <div className="space-y-2">
          <label className="flex items-center gap-2">
            <input type="checkbox" className="w-4 h-4 rounded border-stone-300 text-primary" />
            <span className="text-sm text-stone-700">Stage requires next step date</span>
          </label>
          <label className="flex items-center gap-2">
            <input type="checkbox" className="w-4 h-4 rounded border-stone-300 text-primary" />
            <span className="text-sm text-stone-700">Stage requires follow-up task</span>
          </label>
        </div>
      </div>
    </div>
...
function FormMappingDashboard({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Form Mapping</h1>
        <p className="text-sm text-stone-500">Map form questions to system fields</p>
      </div>
...
function FormMappingDetail({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800">Client Intake Form - Mapping</h1>
          <p className="text-sm text-stone-500">Map each question to a system field</p>
        </div>
        ...
        <div className="divide-y divide-stone-200">
          {mappings.map((mapping, idx) => (
            <div key={idx} className="p-4 flex items-center justify-between">
              <div className="flex-1">
                <p className="font-medium text-stone-800 mb-1">{mapping.question}</p>
                <p className="text-sm text-stone-600">→ {mapping.field}</p>
              </div>
              <div className="flex items-center gap-2">
...
function SubmissionMappingPreview({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Submission Mapping Preview</h1>
        <p className="text-sm text-stone-500">How submission data will be mapped to system</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-stone-200 p-6">
          <h3 className="font-semibold text-stone-800 mb-4">Form Submission</h3>
          <div className="space-y-3">
...
function PortalDataUpdateRules({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Portal Data Update Rules</h1>
        <p className="text-sm text-stone-500">Control self-serve profile updates with approval</p>
      </div>
      ...
      <div className="mt-6 space-y-4">
        <button
          ...
          className="w-full bg-white rounded-lg border border-stone-200 p-6 hover:shadow-md transition-shadow text-left"
        >
          <h3 className="font-semibold text-stone-800 mb-2">Allowed Fields</h3>
          <p className="text-sm text-stone-600">Configure which fields {activeTab}s can request changes for</p>
        </button>
        ...
        <button
          ...
          className="w-full bg-white rounded-lg border border-stone-200 p-6 hover:shadow-md transition-shadow text-left"
        >
          <h3 className="font-semibold text-stone-800 mb-2">Approval Rules</h3>
          <p className="text-sm text-stone-600">Set who approves changes and SLA</p>
        </button>
        ...
        >
          ...
          <p className="text-sm text-stone-600">View sample change request lifecycle</p>
        </button>
      </div>
    </div>
  );
...
function AllowedFieldsConfig({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Allowed Fields - Client</h1>
        <p className="text-sm text-stone-500">Select which fields clients can request changes for</p>
      </div>
...
function ApprovalRulesConfig({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-stone-800">Approval Rules - Client</h1>
        <p className="text-sm text-stone-500">Set who approves client profile changes</p>
      </div>

      <div className="bg-white rounded-lg border border-stone-200 p-6 space-y-4">
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-2">Default Approver</label>
          <select className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm">
            <option>Owner</option>
            <option>Manager</option>
            <option>Admin</option>
        ...
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-2">SLA (days)</label>
          <input
            type="number"
            defaultValue={3}
            className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm"
        ...
        <div>
          <label className="flex items-center gap-2">
            <input type="checkbox" defaultChecked className="w-4 h-4 rounded border-stone-300 text-primary" />
            <span className="text-sm text-stone-700">Send reminder before SLA expires</span>
          </label>
        </div>
      </div>
