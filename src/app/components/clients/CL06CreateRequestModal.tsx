...
export function CL06CreateRequestModal({ isOpen, onClose, onCreate }: CL06CreateRequestModalProps) {
  ...
  const typeClassesByColor: Record<string, { iconBg: string; iconText: string }> = {
    blue: { iconBg: 'bg-primary/10', iconText: 'text-primary' },
    purple: { iconBg: 'bg-primary/10', iconText: 'text-primary' },
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* Header */}
          <div className="px-6 py-4 border-b border-border flex items-center justify-between sticky top-0 bg-[var(--background-2)]">
            <div>
              <h2 className="text-xl font-semibold text-foreground">Create request</h2>
              <p className="text-sm text-muted-foreground">Submit a request to the team</p>
            </div>
            <button
              type="button"
              onClick={onClose}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {requestTypes.map((type) => {
                  ...
                  const typeClasses = typeClassesByColor[type.color] || typeClassesByColor.blue;
                  return (
                    <button
                      key={type.value}
                      type="button"
                    ...
                    >
                      <div className={`w-10 h-10 rounded-lg ${typeClasses.iconBg} flex items-center justify-center`}>
                        <Icon className={`w-5 h-5 ${typeClasses.iconText}`} />
                      </div>
